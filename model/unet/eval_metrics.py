...
def compute_ssim(pred, actual):
    ...
    Degenerate frames (constant variance, all-zero rain fields, etc.) are
    skipped and excluded from the average — they do NOT contribute a 0.
    ...
    # (B, T, H, W): average over time axis first
    if pred.ndim == 4:
        pred   = pred.mean(axis=1)      # -> (B, H, W)
        actual = actual.mean(axis=1)
        scores = [_ssim_2d(pred[i], actual[i]) for i in range(pred.shape[0])]
        return _valid_mean(scores)

    # (T, H, W): average over time
    elif pred.ndim == 3:
        pred   = pred.mean(axis=0)      # -> (H, W)
