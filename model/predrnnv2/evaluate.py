...
CONFIG = {
    ...
    "num_workers": 1,
    "seed":        42,
    "checkpoint":  "../../model_weights/predrnnv2/predrnnv2-best-epoch=3.ckpt",   # ← change to your .ckpt path
...
def collect_predictions(model, loader, device):
    ...
    for x, y in loader:
        ...
        pred = pred.reshape(pred.size(0), 10, 112, 112)
    ...
    targets_raw = targets_raw.permute(0, 1, 4, 2, 3).reshape(-1, 10, 112, 112)
...
def evaluate(preds, targets):
    """
    preds, targets: (N, 10, 112, 112)  — on CPU, linear scale
    """
    results = {}
    CONFIG["csi_thresholds"] = compute_dynamic_thresholds(to_pixel_intensity(targets))

    B, C, H, W = preds.shape                       # C == 10
    pred_seq = preds.view(B, 10, 1, H, W)[:, :, 0]
    y_seq    = targets.view(B, 10, 1, H, W)[:, :, 0]
...
def visualize_predictions(preds, targets, out_dir="./plottings_predrnnv2", num_samples=5):
    ...
    for i in range(n):
        ...
        fig.suptitle(f"PredRNN-V2 — Sample {i+1} | Precipitation (channel 0)",
        ...
        fname = os.path.join(out_dir, f"predrnnv2_sample{i+1:03d}.png")
...
# SAVE RESULTS TO CSV
# ==========================================
def save_results(results, path="validation_results_predrnnv2.csv"):
    import csv
