...
class WindowAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Pad H and W up to the nearest multiple of ws so the reshape is valid.
        # Example: H=W=28, ws=8 → pad to 32; without this the reshape to
        # (B*T, H//ws, ws, W//ws, ws, C) silently requires H%ws == 0.
        pad_h = (ws - H % ws) % ws   # 0 if already divisible
        pad_w = (ws - W % ws) % ws
        if pad_h > 0 or pad_w > 0:
            # F.pad order: (left, right, top, bottom) for last two dims
            x = F.pad(x, (0, 0, 0, pad_w, 0, pad_h))   # pad W then H (last 2 spatial dims)

        _, _, Hp, Wp, _ = x.shape     # padded spatial dims

        x_ = self.norm(x)
        # partition into non-overlapping windows
        x_ = x_.reshape(B*T, Hp//ws, ws, Wp//ws, ws, C).permute(0,1,3,2,4,5)
        ...
        out  = self.to_out(out)
        # unpartition back to padded spatial layout
        out = out.reshape(B*T, Hp//ws, Wp//ws, ws, ws, C).permute(0,1,3,2,4,5)
        out = out.reshape(B, T, Hp, Wp, C)
        # crop back to original (unpadded) spatial size
        out = out[:, :, :H, :W, :]
        # residual on original (unpadded) x — re-slice in case we padded above
        return x[:, :, :H, :W, :] + out
