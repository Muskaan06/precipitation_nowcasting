...
class CuboidSelfAttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        if global_dim_ratio != 1:
            assert separate_global_qkv, "Setting global_dim_ratio != 1 requires separate_global_qkv == True."
        ...
        if self.use_global_vector:
            if self.separate_global_qkv:
                ...
                self.l2g_global_kv_net = nn.Linear(
                    in_features=global_dim_ratio * dim, out_features=dim * 2, bias=qkv_bias
                )
    ...
    def forward(self, x, global_vectors=None):
        ...
        qkv = (
            ...
            .permute(3, 0, 4, 1, 2, 5)
        )  # (3, B, num_heads, num_cuboids, cuboid_volume, head_C)
        ...
        if self.use_relative_pos:
            relative_position_bias = self.relative_position_bias_table[
                ...
                cuboid_volume, cuboid_volume, -1
            )  # (cuboid_volume, cuboid_volume, num_head)
            relative_position_bias = (
                relative_position_bias.permute(2, 0, 1).contiguous().unsqueeze(1)
            )  # num_heads, 1, cuboid_volume, cuboid_volume
            attn_score = (
                attn_score + relative_position_bias
            )  # Shape (B, num_heads, num_cuboids, cuboid_volume, cuboid_volume)
        ...
        if self.use_global_vector:
            ...
            if self.separate_global_qkv:
                ...
                g2l_v = (
                    ...
                    .permute(0, 3, 1, 2, 4)
                )  # (B, num_heads, num_cuboids, cuboid_volume, global_head_C)
                if self.use_global_self_attn:
                    g2g_global_qkv = (
                ...
                q_global, k_global, v_global = (
                    ...
                    .permute(3, 0, 4, 1, 2, 5)
                )  # Shape (3, B, num_heads, 1, N, head_C)
            ...
            l2g_attn_score = l2g_q @ l2g_global_k.transpose(
                -2, -1
            )  # Shape (B, num_heads, num_cuboids, cuboid_volume, N)
            attn_score_l2l_l2g = torch.cat(
                (attn_score, l2g_attn_score), dim=-1
            )  # Shape (B, num_heads, num_cuboids, cuboid_volume, cuboid_volume + N)
            ...
            attn_score_l2l_l2g = self.attn_drop(
                attn_score_l2l_l2g
            )  # Shape (B, num_heads, num_cuboids, x_cuboid_volume, mem_cuboid_volume + K))
            reordered_x = (
                (attn_score_l2l_l2g @ v_l_g).permute(0, 2, 3, 1, 4).reshape(B, num_cuboids, cuboid_volume, self.dim)
            )
            ...
            if self.padding_type == "ignore":
                ...
                g2l_attn_mask = None
            g2l_attn_score = g2l_global_q @ g2l_k.reshape(
                B, self.num_heads, num_cuboids * cuboid_volume, head_C
            ).transpose(
                -2, -1
            )  # Shape (B, num_heads, N, num_cuboids * cuboid_volume)
            if self.use_global_self_attn:
                g2g_attn_score = g2g_global_q @ g2g_global_k.squeeze(2).transpose(-2, -1)
                g2all_attn_score = torch.cat(
                    (g2l_attn_score, g2g_attn_score), dim=-1
                )  # Shape (B, num_heads, N, num_cuboids * cuboid_volume + N)
            ...
            attn_score = self.attn_drop(
                attn_score
            )  # Shape (B, num_heads, num_cuboids, cuboid_volume, cuboid_volume (+ K))
...
class StackCuboidSelfAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        assert len(block_cuboid_size[0]) > 0 and len(block_shift_size) > 0 and len(block_strategy) > 0, (
            f"Format of the block cuboid size is not correct." f" block_cuboid_size={block_cuboid_size}"
        )
...
class CuboidCrossAttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        if use_relative_pos:
            ...
            self.relative_position_bias_table = nn.Parameter(
                torch.zeros(
                    (2 * max_temporal_relative - 1) * (2 * cuboid_hw[0] - 1) * (2 * cuboid_hw[1] - 1), num_heads
                )
            )
            nn.init.trunc_normal_(self.relative_position_bias_table, std=0.02)
    ...
    def forward(self, x, mem, mem_global_vectors=None):
        ...
        assert num_cuboids_mem == num_cuboids, (
            f"Number of cuboids do not match. num_cuboids={num_cuboids}," f" num_cuboids_mem={num_cuboids_mem}"
        )
        ...
        q = (
            ...
            .permute(0, 3, 1, 2, 4)
        )  # Shape (B, num_heads, num_cuboids, x_cuboids_volume, head_C)
        q = q * self.scale
        ...
        if self.use_relative_pos:
            relative_position_bias = self.relative_position_bias_table[
                ...
                x_cuboid_volume, mem_cuboid_volume, -1
            )  # (cuboid_volume, cuboid_volume, num_head)
            relative_position_bias = (
                relative_position_bias.permute(2, 0, 1).contiguous().unsqueeze(1)
            )  # num_heads, 1, x_cuboids_volume, mem_cuboid_volume
            attn_score = (
                attn_score + relative_position_bias
            )  # Shape (B, num_heads, num_cuboids, x_cuboids_volume, mem_cuboid_volume)
        ...
        if self.use_global_vector:
            ...
            l2g_attn_score = l2g_q @ l2g_global_k.transpose(
                -2, -1
            )  # Shape (B, num_heads, num_cuboids, x_cuboid_volume, num_global)
            attn_score_l2l_l2g = torch.cat((attn_score, l2g_attn_score), dim=-1)
            attn_mask_l2l_l2g = F.pad(
                attn_mask, (0, num_global), "constant", 1
            )  # Shape (num_cuboids, x_cuboid_volume, mem_cuboid_volume + num_global)
            v_l_g = torch.cat(
                (v, l2g_global_v.expand(B, self.num_heads, num_cuboids, num_global, head_C)), dim=3
            )  # Shape (B, num_heads, num_cuboids, mem_cuboid_volume + num_global, head_C)
            # local to local and global attention
            attn_score_l2l_l2g = masked_softmax(attn_score_l2l_l2g, mask=attn_mask_l2l_l2g)
            attn_score_l2l_l2g = self.attn_drop(
                attn_score_l2l_l2g
            ...
            reordered_x = (
                (attn_score_l2l_l2g @ v_l_g).permute(0, 2, 3, 1, 4).reshape(B, num_cuboids, x_cuboid_volume, self.dim)
            )
        else:
            attn_score = masked_softmax(attn_score, mask=attn_mask)
            attn_score = self.attn_drop(
                attn_score
            )  # Shape (B, num_heads, num_cuboids, x_cuboid_volume, mem_cuboid_volume)
...
class DownSampling3D(nn.Module):
    ...
    def __init__(
        self, original_size, target_size, in_channels, out_dim, mid_dim=16, act_type="leaky", arch_type="2d_interp_2d"
    ):
        """
        ...
        if self.arch_type == "3d_interp_2d":
            self.inter_conv = nn.Conv3d(
                in_channels=in_channels, out_channels=mid_dim, kernel_size=(3, 3, 3), padding=(1, 1, 1)
            )
            self.act = get_activation(act_type)
        elif self.arch_type == "2d_interp_2d":
            self.inter_conv = nn.Conv2d(
                in_channels=in_channels, out_channels=mid_dim, kernel_size=(3, 3), padding=(1, 1)
            )
            self.act = get_activation(act_type)
        else:
            raise NotImplementedError
    ...
    def forward(self, x):
        ...
        if self.arch_type == "3d_interp_2d":
            ...
            if self.original_size[0] == self.target_size[0]:
                ...
                x = F.interpolate(
                    x.permute(0, 2, 1, 3, 4).reshape(B * T, self.mid_dim, H, W), size=self.target_size[1:]
                )  # Shape (B * T_new, mid_dim, H_new, W_new)
            else:
                ...
                x = x.permute(0, 2, 1, 3, 4).reshape(
                    B * self.target_size[0], self.mid_dim, self.target_size[1], self.target_size[2]
                )
        elif self.arch_type == "2d_interp_2d":
            ...
            if self.original_size[0] == self.target_size[0]:
                ...
                # Use 3D interpolation
                x = F.interpolate(
                    x.reshape(B, T, C_in, H, W).permute(0, 2, 1, 3, 4), size=self.target_size
                )  # Shape (B, mid_dim, T_new, H_new, W_new)
                x = x.permute(0, 2, 1, 3, 4).reshape(
                    B * self.target_size[0], self.mid_dim, self.target_size[1], self.target_size[2]
                )
        else:
        ...
        x = x.reshape(B, self.target_size[0], self.out_dim, self.target_size[1], self.target_size[2]).permute(
            0, 2, 1, 3, 4
        )
        return x
...
class CuboidTransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        if block_units is None:
            block_units = [
                round_to(base_units * int((max(downsample) ** scale_alpha) ** i), 4) for i in range(self.num_blocks)
            ]
        else:
...
class InitialEncoder(nn.Module):
    def __init__(
        ...
        for i in range(num_conv_layers):
            if i == 0:
                ...
                conv_block.append(
                    nn.Conv2d(kernel_size=(3, 3), padding=(1, 1), in_channels=out_dim, out_channels=out_dim)
                )
                conv_block.append(nn.GroupNorm(16, out_dim))
                conv_block.append(get_activation(activation))
    ...
    def reset_parameters(self):
        for m in self.children():
            apply_initialization(
                m, conv_mode=self.conv_init_mode, linear_mode=self.linear_init_mode, norm_mode=self.norm_init_mode
            )
...
class FinalDecoder(nn.Module):
    def __init__(
        ...
        self.upsample = Upsample3DLayer(
            dim=dim, out_dim=dim, target_size=target_thw, kernel_size=3, conv_init_mode=conv_init_mode
        )
        self.reset_parameters()
        ...
        for i in range(num_merge):
            ...
            for j in range(self.num_conv_per_merge_list[i]):
                if j == 0:
                    ...
                    conv_in_dim = out_dim
                conv_block.append(
                    nn.Conv2d(kernel_size=(3, 3), padding=(1, 1), in_channels=conv_in_dim, out_channels=out_dim)
                )
