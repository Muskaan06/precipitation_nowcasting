...
for p in [1, 2, 4, 8, 10]:
    for m in [1, 2, 4, 8, 16, 32]:
        CuboidSelfAttentionPatterns.register(f"video_swin_{p}x{m}", functools.partial(self_video_swin, P=p, M=m))
...
# spatial_lg
for m in [1, 2, 4, 8, 16, 32]:
    CuboidSelfAttentionPatterns.register(f"spatial_lg_{m}", functools.partial(self_spatial_lg_v1, M=m))
...
for k in [2, 4, 8]:
    CuboidSelfAttentionPatterns.register(f"axial_space_dilate_{k}", functools.partial(self_axial_space_dilate_K, K=k))
