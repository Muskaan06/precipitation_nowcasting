...
from model_arch2 import EarthformerModel
...
CONFIG = {
    ...
    "num_workers": 1,
    "seed":        42,
    "checkpoint":  "earthformer_sevir.pt",   # ← update to new .pt path
...
class NPZDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x_raw = torch.from_numpy(arr[:13]).float()      # T_in=13  ← enc_pos_embed.T_embed [13,128]
        y_raw = torch.from_numpy(arr[13:25]).float()    # T_out=12 ← dec_pos_embed.T_embed [12,256]
...
def collect_predictions(model, loader, device):
    ...
    for x, y in loader:
        ...
        pred = pred.reshape(pred.size(0), 12, 112, 112)
        pred = pred[:, :-1, :, :]   # T_out=12, C=1 → flat to (B, 12, 112, 112)
    ...
    targets_raw = targets_raw[:, :-1, :, :]  # T_out=12, C=1
...
def evaluate(preds, targets):
    ...
    pred_seq = preds.view(B, 11, 1, H, W)[:, :, 0]     # (B, 12, H, W) — T_out=12, C=1
...
def visualize_predictions(preds, targets, out_dir="./plottings2", num_samples=5):
    ...
    T_OUT           = 11                                # T_out=12 ← dec_pos_embed.T_embed [12,256]
    N_CH            = 1                                 # C=1      ← dec_final_proj [1, 16]
